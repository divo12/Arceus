...
/**
 * Chorus-style cross-beat resume protocol (dream todo_write → workspace TODO.md).
 * Granting the tool is not enough — without this instruction the model never
 * keeps a checklist. Appended into CONTEXT_MANAGEMENT_RULES for every role.
 */
export const RESUME_DIRECTIVE = `
## RESUME, DON'T RESTART (durable checklist)

Your work can span more than one beat, and a beat can be killed abruptly
when its budget runs out — but your worktree persists. Keep a durable
checklist with the \`todo_write\` tool:

1. List the whole task's steps up front in \`TODO.md\`
   (\`todo_write({item:"..."})\` for each step, unchecked).
2. Check each off THE MOMENT it is done
   (\`todo_write({item:"...", checked:true})\`) — not at the end; the kill
   is abrupt.
3. The FIRST thing you do every beat: \`read\` \`/workspace/TODO.md\` if it
   exists and reconcile it against reality — \`git status\` and your
   artifacts on disk show what ACTUALLY got done — then resume the
   unchecked steps.
4. Never restart from scratch when a checklist and prior work already
   sit in the worktree.
`;
...
4. 3 retries on the same \`error.cause\` → stop: \`task_block\` with
   cause "tool_failure".
5. Track progress via \`todo_write\` into \`TODO.md\` (see RESUME below) —
   the durable checklist, NOT free-text monologue. A checklist update is
   NOT a terminating action: follow it with the real work call, or close
   the task. Never end a beat right after only updating the checklist.
...
\`todo_write\`. Endless skill-gathering is analysis paralysis,
not preparation.
...
- **\`TODO.md\` / checklist in the beat briefing** = your resume point.
  Read it first, reconcile with disk, continue unchecked steps — never
  redo checked items that still match reality.
...
- **A wrap-up notice ("~2 minutes remain")** = checkpoint NOW: finish
  ...
  or \`task_block\`, and \`todo_write\` to sync \`TODO.md\` with where
  you stopped.
...
3. If the task is no longer yours → end the beat. Do NOT re-claim — the next scheduler tick will re-dispatch you with fresh context. Report idle in one sentence (optionally \`todo_write({item:"idle — claim released"})\`).
...
2. If real and unresolved: leave blocked, report idle. Optionally
   \`todo_write({item:"blocked: <reason>"})\`. Do NOT re-claim.
