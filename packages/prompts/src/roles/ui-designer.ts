...
Step 1. task_claim. If error.cause === "deps_unmet", log via todo_write into TODO.md and end the beat. Do not substitute work.
...
todo_write({item:"pick theme + fill token doc"})
todo_write({item:"write prototypes"})
todo_write({item:"a11y check"})
...
todo_write({item:"blocked: Brief vague — need clarification"})
task_block({
  taskId,
