...
\`/workspace\` is a FULL-STACK scaffold: Vite + React 18 + TypeScript + Tailwind 3 + shadcn/ui on the frontend, AND a Hono server tier (\`server/\`) with SQLite-dialect persistence. Locally: \`node:sqlite\` file at \`data/app.db\`. Production (Vercel): same \`server/db.ts\` API against Turso when \`TURSO_DATABASE_URL\` is set (Arceus provisions + injects). \`api/index.ts\` is the Vercel Hono adapter — do not remove it. One \`npm run dev\` serves both the React app and the \`/api/*\` backend on the same port. \`cn()\` helper lives at \`src/lib/utils.ts\`. Components in \`src/components/\` — one file per component.
...
- FULL-STACK (data, APIs, secrets): the product has a real backend.
  - Data that must persist across reloads/users goes through \`server/db.ts\` (define tables in \`migrate()\` + export typed async query helpers) — NOT localStorage. Keep the dual-driver pattern (local file vs Turso); do NOT hardcode only \`node:sqlite\` or only Turso.
  - Add HTTP endpoints under \`/api/*\` in \`server/index.ts\` (Hono). Keep \`api/index.ts\` as the Vercel entry. The frontend calls routes through \`src/lib/api.ts\` (\`fetch("/api/...")\`, same-origin).
  - Secrets/keys + any privileged logic live in \`server/\` (it runs server-side; Arceus injects \`TURSO_*\` + \`ARCEUS_COMPANY_ID\` via \`process.env\` on Vercel and they NEVER reach the browser bundle). Never put a secret in \`src/\` (client) code.
...
- The scaffold is full-stack: Vite + React 18 + TS + Tailwind 3 + shadcn/ui (frontend) + Hono (\`server/\` + \`api/index.ts\` for Vercel) + SQLite dialect (\`server/db.ts\` — local file or Turso). Pre-installed. Do NOT run \`npm create vite\`, scaffold a separate backend (Express/Next/etc.), reconfigure Tailwind, or add a database service — the server + persistence are already wired. Add to them.
