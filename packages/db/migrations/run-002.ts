...
const migration = readFileSync(resolve(__dirname, "002_audit_events.sql"), "utf8");
// Strip BEGIN/COMMIT since postgres.js handles transactions differently
const cleaned = migration.replace(/^\s*BEGIN\s*;/mi, "").replace(/^\s*COMMIT\s*;/mi, "");
await sql.begin(async (tx) => {
  await tx.unsafe(cleaned);
});
console.log("Migration 002_audit_events applied successfully");
await sql.end();
