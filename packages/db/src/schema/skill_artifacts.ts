...
export const skillArtifacts = pgTable(
  ...
  (table) => ({
    ...
    companyActiveSuccessIdx: index("skill_artifacts_company_active_success_idx")
      .on(table.companyId, table.successRate)
      .where(sql`status = 'active'`),
    nameSearchIdx: index("skill_artifacts_name_search_idx").using("gin", sql`${table.name} gin_trgm_ops`),
    statusCheck: check(
