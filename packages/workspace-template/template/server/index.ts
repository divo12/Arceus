 ...
 * server (mounted into Vite in dev via @hono/vite-dev-server; on Vercel via
 * `api/index.ts`). This is where you:
...
// Example resource — a notes table persisted in SQLite (local file or Turso).
// Replace with your product's real data model + endpoints.
app.get("/api/notes", async (c) => c.json(await listNotes()));

app.post("/api/notes", async (c) => {
  const body = await c.req.json().catch(() => ({}));
  const text = typeof body?.text === "string" ? body.text.trim() : "";
  if (!text) return c.json({ error: "text is required" }, 400);
  return c.json(await createNote(text), 201);
});
