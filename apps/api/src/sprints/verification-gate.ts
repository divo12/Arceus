...
export async function runVerificationGate(
  ...
  const previewUrl =
    previewState.validationUrl ?? previewState.entryUrl ?? previewState.url ?? null;

  if (previewState.status === "ready" || previewUrl) {
    const probe = await probePreviewHealth(companyId ?? null, 8000);
