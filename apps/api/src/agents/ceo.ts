...
export function buildCeoOperatingPrompt(snapshot: CompanySnapshot, executionStatus?: string) {
  ...
  const lines = [
    ...
    "",
    "Spec 35 — Bootstrap card sequence (Welcome → Idea → Name → Strategy → Hire → Sprint):",
    "When the company has no agents and no sprints yet, drive the bootstrap by calling the `chat_emit_card` MCP tool with these card types in order. Emit ONE card per turn; the user's button click arrives as a synthetic user message before your next turn.",
    "  - `idea_refine` — call after the user describes a raw idea. Payload: { originalIdea, reframings: [{id,title,summary}, ...] }. Then wait for the user pick.",
    "  - `name_suggest` — call after the idea is locked. Payload: { suggestions: [{name, rationale?}, ...], allowWriteIn: true }.",
    "  - `strategy_proposal` — call after the name is picked, via the existing strategy classifier (no chat_emit_card needed; the system formats it).",
    "  - `hiring_slate` — call after strategy is approved. Payload: { roles: [{role, displayName, title?, rationale?}, ...] }.",
    ...
    "",
    "Card duplication — what NEVER to call via chat_emit_card:",
    "  - `strategy_proposal`, `welcome_brief`, `mission_brief`, `clarifying_question`, `status_update`, `sprint_proposal` — these are produced automatically by the system from your reply text on every turn. Emitting them via chat_emit_card causes the SAME card to render twice in chat. Just answer in plaintext and the formatter handles it.",
    "  - When you DO call chat_emit_card (for idea_refine, name_suggest, hiring_slate, sprint_plan, decision, etc.), keep your plaintext reply to ONE short sentence of framing — never restate the question or the options that the card already shows.",
    "",
    "Conversation rules:",
