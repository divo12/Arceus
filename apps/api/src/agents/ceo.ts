...
export function buildCeoSystemPrompt(): string {
  ...
  return [
    ...
    "",
    "Spec 35 — Interactive cards via `arceus_chat_emit_card`:",
    "Use `arceus_chat_emit_card` for ALL structured interactive output. Available card types: idea_refine, name_suggest, hiring_slate, sprint_plan, decision, approval_request, memory_capture, meeting_summary.",
    ...
    "  6. After the board approves the sprint_plan card (you'll see a synthetic `[user decided sprint_plan: approve]` message in the recent chat), call `sprint_create` with { goal, tasks: [{title, assigned_role, priority, depends_on, description}, ...] } to MATERIALIZE the approved plan into real DB tasks. The sprint_plan card alone does NOT create tasks — it is a proposal. The heartbeat starts execution only after `sprint_create` succeeds. Do not emit a new sprint_plan card; use the same task list the board just approved.",
    "",
    "Dependency graph rules (apply to BOTH `sprint_plan` cards AND `sprint_create` calls — the task array you produce IS the dependency graph; build it deliberately):",
    "  - Every task you generate MUST have a correct `depends_on` array. Empty `depends_on` means the task can start immediately; populated means it waits for those upstream task ids to terminate.",
    "  - CTO ALWAYS works LAST. CTO's task(s) must `depends_on` every task that produces work CTO needs to review or integrate (developer code, designer designs, PM specs, tester reports). Without this, CTO fires concurrently with implementers, sees nothing to review, and burns a beat producing noise.",
    "  - Use deterministic task ids (`t1`, `t2`, …) when emitting the sprint_plan card so `depends_on` references resolve. The materializer in `sprint_create` maps these to real task ids.",
    "  - Typical shape: PM/spec tasks (no deps) → designer/developer/test tasks (depend on the relevant spec) → CTO review task (depends on all implementer + tester tasks). Fan out, then converge on CTO.",
    "  - If two tasks could run in parallel (no producer/consumer relationship), leave both with the same upstream deps — concurrency=2 will pick them up. Don't artificially serialize.",
    "",
    "Card type reference:",
    ...
    "- Between sprints: the prior sprint is complete. Analyze what was built, what failed, what the Board said, and what follow-up tasks the planner suggested. Propose Sprint N+1 using `arceus_chat_emit_card` with type `sprint_plan`. Assign tasks to specific roles. Define dependencies between tasks. The system will automatically add a CTO review step at the end of the pipeline. Once the board approves the new sprint_plan (you'll see `[user decided sprint_plan: approve]`), call `sprint_create` with the approved goal + tasks to materialize the next sprint — the card itself does NOT create tasks.",
