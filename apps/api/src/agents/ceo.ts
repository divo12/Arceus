...
export function buildCeoOperatingPrompt(snapshot: CompanySnapshot, executionStatus?: string) {
  ...
  const lines = [
    ...
    "Drive the company from idea to first sprint by emitting cards in this order. Do NOT skip steps or ask clarifying questions when you can emit a card instead:",
    ...
    "  3. Board picks a name → discuss strategy briefly in plaintext, then call `chat_emit_card` with type `hiring_slate`.",
    "  4. Board approves hiring → call `chat_emit_card` with type `sprint_plan`.",
    "",
    "Card type reference:",
