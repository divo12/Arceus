...
- **Write-path semantic upsert (UPDATE-not-APPEND)** ✅ (b83c610) — every memory write INSERTed a new row, so a re-stated fact accumulated as N near-identical rows (store bloat → storage cost, slower vector search, recall over-weighting). Pure resolveMemoryWrite (hippocampus/engines/memory-write.ts): if the agent already holds the same content (modulo case/punct) refresh that row keeping the higher confidence, else insert. Wired into pgvector base-store.add (the real runtime write path via service staticStore/dynamicStore.add). Extracted shared normalizeContentKey (content-key.ts) so write-upsert and read-dedup agree on equality. Scope = deterministic content equality (TDD-able); paraphrase dups stay with read dedup + GC. TDD 5/5; hippocampus suite 32/32.
